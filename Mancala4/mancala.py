...
class Mancala(object):
    ...
    #   name            :   string, is used to call the data of bias and weights 
        ...
        self.spielfeld          = np.array([4,4,4,4,4,4,4,4,4,4,4,4,0,0])  #Spielfeld (zustand)
    ...
    def reset(self):
        self.spielfeld          = np.array([4,4,4,4,4,4,4,4,4,4,4,4,0,0])
        self.spieler1           = True
    ...
    def play(self):                                  # überprüfen
        ...
        reward_liste   = [0.0]
        while not(np.array_equal(self.spielfeld[0:6] ,[0,0,0,0,0,0]) or np.array_equal(self.spielfeld[6:12] ,[0,0,0,0,0,0]) or self.spielfeld[12]>24 or self.spielfeld[13]>24):
    ...
    def play_vs_rand(self):                                  # überprüfen
        ...
        while not(np.array_equal(self.spielfeld[0:6] ,[0,0,0,0,0,0]) or np.array_equal(self.spielfeld[6:12] ,[0,0,0,0,0,0]) or self.spielfeld[12]>24 or self.spielfeld[13]>24):
            ...
            reward_liste.append(reward)
            #reward_liste[-2] -= reward_liste[-1]
            if not(np.array_equal(self.spielfeld[0:6] ,[0,0,0,0,0,0]) or np.array_equal(self.spielfeld[6:12] ,[0,0,0,0,0,0]) or self.spielfeld[12]>24 or self.spielfeld[13]>24):
                self.spieler1          = not self.spieler1
