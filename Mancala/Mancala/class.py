...
class Mancala(object):
    ...
    def greedy_action(self, spielfeld):
        #choose the action that will end to the highest q-value (according to the neuronal network)
    ...
    def get_next_action(self, spielfeld):
        if random.random() > self.exploration_rate:
            ...
            return #self.random_action()            Hier müssen wir uns nochmal genau überlegen, wo wir die regeln einbauen wollen
    ...
    def train_net(self, iterations):
        for i in range(iterations):
            #spielfeld_liste =                      Hier benötigen wir dann noch geeignete Spielfunktionen
            training_data = create_training_data(spielfeld_liste)
