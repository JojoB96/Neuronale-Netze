# -*- coding: utf-8 -*-
"""
Created on Tue Sep  1 20:46:15 2020
...
class Testspiel(object):
    ...
    #   name            :   string, is used to call the data of bias and weights 
        ...
        self.spielfeld          = np.array([1,1,5,1,1,1,1,1,5,1,1,1,0,0])  #Spielfeld (zustand)
    ...
    def reset(self):
        self.spielfeld          = np.array([1,1,5,1,1,1,1,1,5,1,1,1,0,0])
        self.spieler1           = True
    ...
    def get_next_action(self, spielfeld):
        if not self.spieler1:
            spielfeld = self.get_turned_spielfeld(spielfeld)
    ...
    def get_spielfeld_and_reward_after_action(self, spielfeld, action):
        ...
        for b in range(bohnen):
            b = bohnen - b 
            if (tmp_spielfeld[(action+b) %12] == 2):
        ...
        #if sum(tmp_spielfeld[0:6])> sum(tmp_spielfeld[6:12]):
         #   reward+=0.5
        #if reward == 0:
         #   reward-=0.1
        #...#
        ...
        if tmp_spielfeld[12] >10:
            tmp_spielfeld[12] += sum(tmp_spielfeld[0:6]) + sum(tmp_spielfeld[6:12])
            tmp_spielfeld[0:6]  = [0,0,0,0,0,0]
            tmp_spielfeld[6:12] = [0,0,0,0,0,0]
            reward += self.rewards[1]*2
            
        elif tmp_spielfeld[13] > 10:
       ...
       # reward = tmp_spielfeld[12]-tmp_spielfeld[13]
    ...
    def play(self):      
        self.spieler1 =   not self.spieler1                          # überprüfen
        Spielfeldliste = [deepcopy(self.spielfeld)]
        reward_liste   = [0.0]
        while not(np.array_equal(self.spielfeld[0:6] ,[0,0,0,0,0,0]) or np.array_equal(self.spielfeld[6:12] ,[0,0,0,0,0,0]) or self.spielfeld[12]>10 or self.spielfeld[13]>10):
